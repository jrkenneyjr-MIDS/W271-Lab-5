...
plot.hist4  = ggplot(traffic, aes(x = totfatpvm)) + scale_x_continuous(name ="Total/Mile Driven") + h + t + t
...
plot.hist9  = ggplot(traffic, aes(x = wkndfatrte)) + scale_x_continuous(name ="Weekend/Capita") + h + t + t
...
plot.map1 = qplot(long, lat, data=traffic.map, geom="polygon", fill=totfat, group=group)
plot.map2 = qplot(long, lat, data=traffic.map, geom="polygon", fill=nghtfat, group=group)
plot.map3 = qplot(long, lat, data=traffic.map, geom="polygon", fill=wkndfat, group=group)
plot.map4 = qplot(long, lat, data=traffic.map, geom="polygon", fill=totfatpvm, group=group)
plot.map5 = qplot(long, lat, data=traffic.map, geom="polygon", fill=nghtfatpvm, group=group)
plot.map6 = qplot(long, lat, data=traffic.map, geom="polygon", fill=wkndfatpvm, group=group)
plot.map7 = qplot(long, lat, data=traffic.map, geom="polygon", fill=totfatrte, group=group)
plot.map8 = qplot(long, lat, data=traffic.map, geom="polygon", fill=nghtfatrte, group=group)
plot.map9 = qplot(long, lat, data=traffic.map, geom="polygon", fill=wkndfatrte, group=group)
...
head(traffic.state.agg)
traffic.map = merge(states, traffic.state.agg, by="state")
tail(traffic.map)
...
#Question 2
****************
2. How is the our dependent variable of interest totfatrte defined? What is the average of this variable in
each of the years in the time period covered in this dataset? Estimate a very simple regression model of
totfatrte on dummy variables for the years 1981 through 2004. What does this model explain? Describe
what you find in this model. Did driving become safer over this period? Please provide a detailed
explanation.
****************
