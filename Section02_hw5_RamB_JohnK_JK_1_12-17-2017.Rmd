...
```
**Observations:**
Doesn't look like a Poisson Distribution is appropriate, otherwise we would see that the mean and the variance are similar.
...
```
**Observations:**
Which variables would benefit from potential log/power transformations?

Positive skew on all three variable types (tot, pvm, rte).  Total per capita seems to approach normality better than any other.
...
```
**Observations:**
Good agreement between state and rates overall, at night, and during the weekend

**Total Fatalaties: **
Variance is proportional to the mean of the state
There are a few outliers that would significantly affect the results (California and Texas)

**Per Vehicle Mile:**
Observations look constant with similar variance.  No outliers.
...
grid.arrange(plot.bp1, plot.bp2, plot.bp3, plot.bp4, plot.bp5, plot.bp6, plot.bp7, 
             plot.bp8, plot.bp9, nrow = 3, ncol = 3, top = quote("Boxplots of State Fatalaties: \n1980 - 2004"))
```
**Observations:  **

**Total:**
Top outliers look to have decreased over time, but average looks almost unchanged over time.

**PVM:**
Significant decline over time in average rate per vehicle mile driven.  Variance looks to be proportional to mean. 


**RTE:**
Significant decline over time in average per capita.  Variance does not look to be proportional to mean. 
...
**TOT:**
California, Texas, and Florida have the highest overall total fatalaties averaged over all years observed.  It's apparent that these observations are outliers and will skew our modeling.
...
**RTE:**
Wyoming and Montana show higher per capita rates.  There is some regional influence, but of the three, this is probably the best variable to model.

**Overall Univariate Conclusions:**
Let's go with the $totfatrte$ variable.  It looks normally distributed, doesn't have outliers, nor does there seem to be regional correlation, which might lead to omitted variable bias.  There also doesn't look to be much serial correlation or heteroskedasticity over time, which means we may be able to get reasonable results from a pooled OLS.
...
```{r}
sum_by_year = aggregate(traffic[,c('bac08','bac10','sbprim','sbsecon','perse','sl70plus','gdl', "zerotol")], traffic['year'], FUN=sum)
sum_by_state = aggregate(traffic[,c('bac08','bac10','sbprim','sbsecon','perse','sl70plus','gdl', 'zerotol')], traffic['state'], FUN=sum)
#install.packages("GGally")
library(GGally)
ggpairs(sum_by_year) + ggtitle(label = "Number of States with Law vs. Years")
ggpairs(sum_by_state) + ggtitle(label = "Number of Years State Had Law From 1980-2004 vs. State")
```

**Interactions Year Observations Summed Over All States:**
These data points indicate trends for the entire country and give us a sense of the adoption of each law over time.  We see a strong time dependence between each of the variables and time.  Although the correlation between time and $bac10$ is low, it's evident that a quadratic would do an excellent job of predicting the number of states with the law at any given year.  When comparing scatters of any two variables, we can examine the covariance between two variables.  For example, there seems to be strong covariance between the $gdl$ and $sbprim$ covariates.  That is to say, for a given year, if we see a relatively high number of states with $gdl$ laws, we would also expect there to be a relatively high number of states with $sbprim$ laws for that year.


**Interactions with State Observations Summed Over Time:**
...
ADD:  Scatterplot matrix of avg tot rate, tot pvm, and tot fat vs. sum variables (bac, per se )

```{r}
sum_by_year = aggregate(traffic[,c("totfatrte",'bac08','bac10','sbprim','sbsecon','perse','sl70plus','gdl', "zerotol")], traffic['year'], FUN=sum)
sum_by_state = aggregate(traffic[,c("totfatrte",'bac08','bac10','sbprim','sbsecon','perse','sl70plus','gdl', 'zerotol')], traffic['state'], FUN=sum)
library(GGally)
ggpairs(sum_by_year) + ggtitle(label = "Number of States with Law vs. Years")
ggpairs(sum_by_state) + ggtitle(label = "Number of Years State Had Law From 1980-2004 vs. State")
```
...
```{r}
sum_by_year = aggregate(traffic[,c("totfatrte","statepop", "vehicmiles", "vehicmilespc","unem", "perc14_24", "minage")], traffic['year'], FUN=mean)
sum_by_state = aggregate(traffic[,c("totfatrte","statepop", "vehicmiles", "vehicmilespc","unem", "perc14_24", "minage")], traffic['state'], FUN=mean)
library(GGally)
ggpairs(sum_by_year) + ggtitle(label = "Average Across State vs. Year")
ggpairs(sum_by_state) + ggtitle(label = "Average of Demographic 1980-2004 vs. State")
